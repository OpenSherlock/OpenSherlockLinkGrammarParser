...
public class PPKnowledge {
    ...
    public PPKnowledge(ParseOptions opts, String dictname, String path,Dictionary dict) throws IOException {
       System.out.println("PPKOPEN");
        Reader f = dict.dictopen(opts, dictname, path);
        if (f == null)
    ...
    private void initialize_set_of_links_starting_bounded_domain() {
        ...
        set_of_links_starting_bounded_domain = PPLinkset.PPLinkset_open(IParserConstants.PP_MAX_UNIQUE_LINK_NAMES);
    ...
    private PPLinkset read_link_set(ParseOptions opts, String label) {
        ...
        if (!lt.pp_lexer_set_label(label)) {
            if (opts.verbosity > 0)
                opts.out.println("PP warning: ResultLink set " + label + " not defined: assuming empty.");
            n_strings = 0;
        } else
    ...
    private void read_starting_link_table() {
        ...
        if ((n_tokens % 2) != 0)
            throw new RuntimeException("post_process: ResultLink table must have format [<link> <domain name>]+");
