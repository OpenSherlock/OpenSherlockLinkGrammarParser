 ...
 * A Main option is associated with each sentence and each linkage.
 * It is important to keep track of which Main option instance object
 ...
 * <li> disjunct_cost = IParserConstants.MAX_DISJUNCT_COST;
 * <li> min_null_count = 0;
 * <li> max_null_count = 0;
 * <li> null_block = 1;
 * <li> islands_ok = false;
...
public class ParseOptions {
    ...
    public ParseOptions() {
        ...
        verbosity = 0;
        linkage_limit = 100;
        disjunct_cost = IParserConstants.MAX_DISJUNCT_COST;
        min_null_count = 0;
        max_null_count = 0;
        null_block = 1;
        islands_ok = false;
    ...
    public void print_expression(Exp n) {
        ...
        if (n.type == IParserConstants.CONNECTOR_TYPE) {
            for (i = 0; i < n.cost; i++)
                out.print("[");
            ...
            for (el = n.l; el != null; el = el.next) {
                ...
                if (el.next != null) {
                    if (n.type == IParserConstants.AND_type)
                        out.print(" & ");
                    if (n.type == IParserConstants.OR_type)
                        out.print(" | ");
                }
            }
            for (i = 0; i < n.cost; i++)
