...
public class ParseInfo {
    ...
    ParseSet parse_set;
    int N_words;
    Disjunct chosen_disjuncts[] = new Disjunct[IParserConstants.MAX_SENTENCE];
    int N_links;
    Link link_array[] = new Link[IParserConstants.MAX_LINKS];
    ...
    void list_random_links(ParseSet set) {
        ...
        if (pc == null) {
            throw new RuntimeException("Couldn't get a random Main choice");
        }
        issue_links_for_choice(pc);
    ...
    void issue_link(Disjunct ld, Disjunct rd, Link link) {
        if (N_links > IParserConstants.MAX_LINKS - 1) {
    ...
    int link_cost() {
        /* computes the cost of the current Main of the current sentence */
    ...
    void build_digraph() {
        ...
        for (link = 0; link < N_links; link++) {
            lp = link_array[link];
            i = lp.lc.label;
            if (i < IParserConstants.NORMAL_LABEL) { /* one of those special links for either-or, etc */
                continue;
            }
            lol = new ListOfLinks();
            ...
            lol.word = lp.r;
            i = lp.lc.priority;
            if (i == IParserConstants.THIN_priority) {
                lol.dir = 0;
            } else if (i == IParserConstants.DOWN_priority) {
                lol.dir = 1;
            } else {
                lol.dir = -1;
            }
    ...
    DISNode build_DIS_CON_tree() {
        ...
        for (w = 0; w < N_words; w++)
            dfs_height[w] = 0;
        for (w = 0; w < N_words; w++)
            height_dfs(w, IParserConstants.MAX_SENTENCE);
    ...
    ParseSet parse_set(Sentence sent,Disjunct ld, Disjunct rd, int lw, int rw, Connector le, Connector re, int cost, ParseOptions opts) {
        ...
        /* returns null if there are no ways to Main, or returns a pointer
           to a set structure representing all the ways to Main */
    ...
    int x_hash(int lw, int rw, Connector le, Connector re, int cost) {
        ...
        i = i + (i << 1) + MyRandom.randtable[(((le==null?0:le.hashCode()) + i) % (x_table_size + 1)) & (IParserConstants.RTSIZE - 1)];
        i = i + (i << 1) + MyRandom.randtable[(((re==null?0:re.hashCode()) + i) % (x_table_size + 1)) & (IParserConstants.RTSIZE - 1)];
    ...
    static ListOfLinks word_links[] = new ListOfLinks[IParserConstants.MAX_SENTENCE]; /* ptr to l.o.l. out of word */
    static int dfs_root_word[] = new int[IParserConstants.MAX_SENTENCE]; /* for the depth-first search */
    static int dfs_height[] = new int[IParserConstants.MAX_SENTENCE]; /* to determine the order to do the root word dfs */
    static Integer height_perm[] = new Integer[IParserConstants.MAX_SENTENCE]; /* permute the vertices from highest to lowest */
