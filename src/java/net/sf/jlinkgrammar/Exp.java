...
public class Exp {
    ...
    Clause build_Clause(int cost_cutoff) {
        ...
        if (type == IParserConstants.AND_type) {
            c1 = new Clause();
            c1.c = null;
            c1.next = null;
            c1.cost = 0;
            ...
            c = c1;
        } else if (type == IParserConstants.OR_type) {
        ...
        } else if (type == IParserConstants.CONNECTOR_TYPE) {
            c = new Clause();
            c.c = build_terminal();
            c.cost = 0;
            c.maxcost = 0;
    ...
    int size_of_expression() {
        ...
        int size;
        ExpList el;
        if (type == IParserConstants.CONNECTOR_TYPE)
            return 1;
        size = 0;
    ...
    void insert_connectors(int dir) {
        ...
        dummy.label = IParserConstants.NORMAL_LABEL;
        dummy.priority = IParserConstants.THIN_priority;
    ...
    static Exp copy_Exp(Exp e) {
        ...
        if (e == null)
            return null;
        n = new Exp(e);
        if (e.type != IParserConstants.CONNECTOR_TYPE) {
    ...
    void build_connector_set_from_expression(ConnectorSet conset) {
        ...
        if (type == IParserConstants.CONNECTOR_TYPE) {
            ...
            c.init_connector();
            c.string = string;
            c.label = IParserConstants.NORMAL_LABEL; /* so we can use match() */
