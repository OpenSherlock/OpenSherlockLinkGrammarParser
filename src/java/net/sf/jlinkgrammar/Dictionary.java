...
public class Dictionary {
    ...
    private PPLinkset linkSet = new PPLinkset();
    public String name;
    ...
    boolean rand_table_inited = false;
    
    /**
     * Return the dictionary root node
     * @return
     */
     ...
     *
     * @param opts the set of Main options is copied into the Sentence object and ResultLinkage Object as well as the GlobalBean Object
     ...
     * @throws IOException
     */
    public Dictionary(ParseOptions opts, String dict_name, String pp_name, String cons_name,
    		String affix_name, String basePath, LinkGrammarEnvironment env)
        throws IOException {
     	environment = env;
     	_basePath = basePath;
        Dictionary dict;
        ...
        if (!openDictionary(dictionary_path_name)) {
            throw new RuntimeException("Could not open dictionary " + dict_name);
        }

        read_dictionary();

        left_wall_defined = boolean_dictionary_lookup(IParserConstants.LEFT_WALL_WORD);
        right_wall_defined = boolean_dictionary_lookup(IParserConstants.RIGHT_WALL_WORD);
        postprocessor = post_process_open(opts, name, pp_name);
        constituent_pp = post_process_open(opts, name, cons_name);

        affix_table = null;
        if (affix_name != null) {
            affix_table = new Dictionary(opts, affix_name, null, null, null, basePath,env);
        }

        unknown_word_defined = boolean_dictionary_lookup(IParserConstants.UNKNOWN_WORD);
        use_unknown_word = true;
        capitalized_word_defined = boolean_dictionary_lookup(IParserConstants.PROPER_WORD);
        pl_capitalized_word_defined = boolean_dictionary_lookup(IParserConstants.PL_PROPER_WORD);
        hyphenated_word_defined = boolean_dictionary_lookup(IParserConstants.HYPHENATED_WORD);
        number_word_defined = boolean_dictionary_lookup(IParserConstants.NUMBER_WORD);
        ing_word_defined = boolean_dictionary_lookup(IParserConstants.ING_WORD);
        s_word_defined = boolean_dictionary_lookup(IParserConstants.S_WORD);
        ed_word_defined = boolean_dictionary_lookup(IParserConstants.ED_WORD);
        ly_word_defined = boolean_dictionary_lookup(IParserConstants.LY_WORD);
        max_cost = 1000;

        if ((dict_node = dictionary_lookup(IParserConstants.ANDABLE_CONNECTORS_WORD)) != null) {
            andable_connector_set = dict_node.exp.connector_set_create();
        } else {
            andable_connector_set = null;
        }
        if ((dict_node = dictionary_lookup(IParserConstants.UNLIMITED_CONNECTORS_WORD)) != null) {
     ...
     *     as the full path.
     * <p>
     * @param opts the Main options. These are kept in many places, use care!
    ...
    Postprocessor post_process_open(ParseOptions opts, String dictname, String path) throws IOException {
        ...
        pp.knowledge = new PPKnowledge(opts, dictname, path, this);
        pp.set_of_links_of_sentence = linkSet.PPLinkset_open(1024);
        pp.set_of_links_in_an_active_rule = linkSet.PPLinkset_open(1024);
     ...
     * @param dict_path_name
     * @return
     * @throws IOException
     */
    private boolean openDictionary(String dict_path_name) throws IOException {
 //   	System.out.println("Dictionary.open_dictionary "+dict_path_name);
    ...
    /**
     * Read the dictionary into memory. This must be preceded by opening the dictionary
     * and setting the file pointer using open_dictionary.
     * @see Dictionary#openDictionary(String)
     * @throws IOException
     */
    ...
    */
    ...
    /**
     *  @return <code>true</code> if <code>s</code> is in the dictionary
     *  @param s
     *  @return
     */
    ...
    /**
     * Returns <code>false</code> if <code>s</code>not a correctly formed idiom string.
     *<p>
     *      correct such string:
     * <p>
     *      () contains no "."
     ...
     */
    boolean is_idiom_string(String s) {
     ...
     */
    boolean is_idiom_word(String s) {
        /* returns true if this is a word ending in ".Ix", where xNode is a number. */
        return (numberfy(s) != -1);
    }
     ...
     */
    boolean is_initials_word(String word) {
     ...
     * xNode is a number, return xNode.  Return -1 if not of this form.
     * @param s
     * @return
     */
    int numberfy(String s) {
    ...
    /** 
     * return <code>true</code> if <code>s</code> is a sequence of digits. 
     * @param s
     * @return
     */
    boolean is_idiom_number(String s) {
    ...
    /** 
     * Returns <code>true</code> <code>s</code> contains an underbar character.
     * @param s
     * @return
     */
    boolean contains_underbar(String s) {
    ...
    /** 
     * Get the next character from the input, eliminating comments.
    ...
    /** 
     * read the next token from the input into token 
     * @return
     * @throws IOException
     */
    boolean advance() throws IOException {
        ...
        token.setLength(0);
        if (already_got_it != '\0') {
            is_special = (SPECIAL.indexOf(already_got_it) >= 0);
            if (already_got_it >= 0) {
    ...
    /** 
     * returns <code>true</code if this token is a special token and it is equal to connector 
     * @param c
     * @return
     */
    ...
    Exp connector() throws IOException {
        ...
        if (token.charAt(i) != '+' && token.charAt(i) != '-') {
            ...
            n.type = IParserConstants.CONNECTOR_TYPE;
            n.cost = 0;
        }
        if (!advance()) {
            return null;
    ...
    Exp make_unary_node(Exp e) {
        ...
        Exp n;
        n = Exp_create();
        n.type = IParserConstants.AND_type; /* these must be AND types */
        n.cost = 0;
        n.l = new ExpList();
        n.l.next = null;
        n.l.e = e;
    ...
    Exp make_optional_node(Exp e) {
        ...
        ExpList el, elx;
        n = Exp_create();
        n.type = IParserConstants.OR_type;
        n.cost = 0;
        n.l = el = new ExpList();
     ...
     */
    ///////////////////////////////////////////////////////
    //TODO: possibly somewhere in here, we make loops of expression lists
    Exp restricted_expression(boolean and_ok, boolean or_ok) throws IOException {
        ...
        if (is_equal('&') || (token.toString().equals("and"))) {
            ...
            ell.e = nl;
            elr.e = nr;
            n.type = IParserConstants.AND_type;
            n.cost = 0;
    ...
    */
    ...
    void insert_list(DictNode p, int l) throws IOException {
        ...
        if (contains_underbar(dn.string)) {
            ...
            opts.out.println("    Words ending \".Ix\" (xNode a number) are reserved for idioms.");
    ...
    /** Starting with the current token Main one dictionary entry.     
    ...
    void insert_idiom(DictNode dn) throws IOException {
        ...
        nc.dir = '-';
        nc.multi = false;
        nc.type = IParserConstants.CONNECTOR_TYPE;
        nc.cost = 0;
        ...
        ell.e = nc;
        elr.e = no;
        n1.type = IParserConstants.AND_type;
        n1.cost = 0;
    ...
    /**
     * <ul>
     * <li>  (1) opens the word file and adds it to the word file list</li>
     * <li>  (2) reads in the words</li>
     * <li>  (3) puts each word in a {@link DictNode}</li>
     * <li>  (4) links these together by their left pointers at the front of the list pointed to by <code>dn</code></li>
     * <li>  (5) returns a pointer to the first of this list</li>
     * <ul>
     * @param dn
     * @param filename
     * @throws IOException
   ...
   /**
     * <p>This function is used to open a dictionary file or a word file,
     ...
     *   DICTPATH environment variable, followed by the DEFAULTPATH</p>
     *   @param opts
     *   @param dictname
     *   @param filename
     *   @return
     *   @throws IOException
     */
    public Reader dictopen(ParseOptions opts, String dictname, String filename) throws IOException {
    	environment.logDebug("Dictionary.dictopen- "+dictname+" "+filename);
    	//data/link/4.0.dict data/link/4.0.dict
 ...
 //   	environment.logDebug("Dictionary.dictopen-a "+fulldictpath);
      if (filename.endsWith(".dict") ||
    	  filename.endsWith("knowledge") ||
    	  filename.endsWith("affix"))
        	fulldictpath = filename;
        else
        	fulldictpath =  _basePath+filename;
    	environment.logDebug("Dictionary.dictopen-1 "+fulldictpath);
    	//data/lin:data/
        /* now fulldictpath is our dictpath, where each entry is followed by a ":"
           including the last one */
    	System.out.println("OPENING "+fulldictpath);
        File f;
       // if (fulldictpath.endsWith(".dict")) {
            f = new File(fulldictpath);
            if (f.exists()) {
            	environment.logDebug("Dictionary.dictopen-2 "+fulldictpath);
//                if (opts.verbosity >2 ) opts.out.println("   Opening " + completename);
                return new FileReader(f);
            }      	
      //  }
        filenamelen = filename.length();
        len = fulldictpath.length() + filenamelen + 1 + 1;
        oldpos = 0;
    	environment.logDebug("Dictionary.dictopen-3 "+fulldictpath);
        while ((pos = fulldictpath.indexOf(':', oldpos)) >= 0) {
        		completename = fulldictpath.substring(oldpos, pos) + "/" + filename;
        	environment.logDebug("Dictionary.dictopen-4 "+completename);
    ...
    /**
      ...
      * then allocate string space for it and return a pointer to it.
      * @return
      */
    ...
    /**
      ...
      *     This is the same as s, but with a postfix of ".Ix", where xNode is an
      *     appropriate number.  xNode is the minimum number that distinguishes
    ...
    /**
     * Look for words that end in ".Ix" where xNode is a number.
     * Return the largest xNode found.
     * @param d
     * @return
     */
    ...
    /**
     * Tear the idiom string apart.<br />
     * Destroys the string s, but does not free it.<br />
     *  Put the parts into a list of DictNodes (connected by their right pointers)<br />
     *  Sets the string fields of these DictNodes pointing to the
     *  fragments of the string s.  Later these will be replaced by
     *  correct names (with .Ix suffixes).<br />
     * The list is reversed from the way they occur in the string.<br />
    ...
    StringBuffer current_name = new StringBuffer("AAAAAAAA");
