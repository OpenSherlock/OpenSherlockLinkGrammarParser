...
/**
 *<p>  Routines for creating and destroying processing Sentences. It contains
 *  several its own copy of objects such as Dictionary, ParseInfo, ResultLinkage, LinkageInfo,
 ...
 *  can be modified independently of the actual length of the table. </p>
 *<p>
 ...
 *<p>
 *<p>One last note: Everything is referenced via integer indexes into arrays rather than
 ...
 * checking and overall program safety. - jlr</p>
 * <p>
 * The most important
...
public class Sentence {
	...
	//supporting beans
	private GlobalBean myBean = new GlobalBean();
   /**
     ...
     */
    public Word word[] = new Word[IParserConstants.MAX_SENTENCE];
    /**
     ...
     */
    public int num_linkages_post_processed=0;
    /**
     * number with no pp violations
     */
    public int num_valid_linkages=0;
    /**
    ...
    public int post_quote[] = new int[IParserConstants.MAX_SENTENCE];
    ...
    public Sentence(LinkGrammarEnvironment env, String input_string, Dictionary dict, ParseOptions opts) {
    	environment = env;
    	//////////////////////////////////////
    	//TODO thread safety with dict.lookup_list
        dict.lookup_list = null;
        myLabel = IParserConstants.NOT_LABEL;
        if (opts.parse_options_get_batch_mode()) {
        	myLabel = myBean.strip_off_label(input_string);
        }

        this.dict = dict;
        //lenth should
        length = input_string.length(); //0;
        ...
        //the workhorse reading this sentence
        build_sentence_expressions(opts);

        patch_array = new PatchElement[IParserConstants.MAX_LINKS];
    ...
    private boolean sentence_in_dictionary() {
        ...
        for (w = 0; w < length; w++) {
            ...
            if (!dict.boolean_dictionary_lookup(s)
                && !(Character.isUpperCase(s.charAt(0)) && dict.is_s_word(s) && dict.pl_capitalized_word_defined)
                && !(Character.isUpperCase(s.charAt(0)) && dict.capitalized_word_defined)
                && !(dict.ishyphenated(s) && dict.hyphenated_word_defined)
                && !(dict.is_number(s) && dict.number_word_defined)
                && !(dict.is_ing_word(s) && dict.ing_word_defined)
                && !(dict.is_s_word(s) && dict.s_word_defined)
                && !(dict.is_ed_word(s) && dict.ed_word_defined)
                && !(dict.is_ly_word(s) && dict.ly_word_defined)) {
                if (ok_so_far) {
    ...
    public boolean separate_sentence(String s, ParseOptions opts) {
        ...
        for (int i = 0; i < IParserConstants.MAX_SENTENCE; i++)
            post_quote[i] = 0;
        length = 0;

        if (dict.left_wall_defined)
            if (!issue_sentence_word(IParserConstants.LEFT_WALL_WORD))
                return false;
        ...
        if (dict.right_wall_defined)
            if (!issue_sentence_word(IParserConstants.RIGHT_WALL_WORD))
                return false;
    ...
    private boolean separate_word(String w, int wend, boolean is_first_word, boolean quote_found, ParseOptions opts) {
        ...
        int r_stripped[] = new int[IParserConstants.MAX_STRIP]; /* these were stripped from the right */
        ...
        if (dict.affix_table != null) {
            start_dn = DictNode.list_whole_dictionary(dict.affix_table.getRoot(), null);
        ...
        for (n_r_stripped = 0; n_r_stripped < IParserConstants.MAX_STRIP; n_r_stripped++) {

            word = new StringBuffer(w.substring(0, wend));
            if (word.length() == 0)
                break; /* it will work without this */

            if (dict.boolean_dictionary_lookup(word.toString()) || dict.is_initials_word(word.toString()))
                break;
    ...
    private boolean issue_sentence_word(String s) {
        ...
        if (s.length() == 0)
            return true;
        if (s.length() > IParserConstants.MAX_WORD) {
            throw new RuntimeException(
                ". The word \""
                    + s
                    + "\" is too long.\n"
                    + "A word can have a maximum of "
                    + IParserConstants.MAX_WORD
                    + " characters.\n");
        }

        if (length == IParserConstants.MAX_SENTENCE) {
     ...
     *
     *
     * @param opts - not used everything comes from IParserConstants. TODO - Fix or drop
     */
    public void build_sentence_expressions(ParseOptions opts) {
        environment.logDebug("Sentence.build_sentence-");
        ...
        for (i = 0; i < length; i++) {
            ...
            if (dict.boolean_dictionary_lookup(s)) {
                ...
                Character.isUpperCase(s.charAt(0)) && dict.is_s_word(s) && dict.pl_capitalized_word_defined) {
                special_string(i, IParserConstants.PL_PROPER_WORD);
            } else if (Character.isUpperCase(s.charAt(0)) && dict.capitalized_word_defined) {
                special_string(i, IParserConstants.PROPER_WORD);
            } else if (dict.is_number(s) && dict.number_word_defined) {
                /* we know it's a plural number, or 1 */
                /* if the string is 1, we'll only be here if 1's not in the dictionary */
                special_string(i, IParserConstants.NUMBER_WORD);
            } else if (dict.ishyphenated(s) && dict.hyphenated_word_defined) {
                /* singular hyphenated */
                special_string(i, IParserConstants.HYPHENATED_WORD);
            } else if (dict.is_ing_word(s) && dict.ing_word_defined) {
                guessed_string(i, s, IParserConstants.ING_WORD);
            } else if (dict.is_s_word(s) && dict.s_word_defined) {
                guessed_string(i, s, IParserConstants.S_WORD);
            } else if (dict.is_ed_word(s) && dict.ed_word_defined) {
                guessed_string(i, s, IParserConstants.ED_WORD);
            } else if (dict.is_ly_word(s) && dict.ly_word_defined) {
                guessed_string(i, s, IParserConstants.LY_WORD);
    ...
    private void guessed_string(int i, String s, String type) {
        ...
        if (dict.boolean_dictionary_lookup(type)) {
            ...
            if (dict.is_s_word(s)) {
                for (; e != null; e = e.next) {
                    t = e.string.indexOf('.');
                    System.out.println("XXX "+e.string+" "+t+" "+s);
                    if (t >= 0 && t < s.length()) {  //Required patch
                ...
                if (dict.is_ed_word(s)) {
                    e.string = s + "[!].v";
                } else if (dict.is_ing_word(s)) {
                    e.string = s + "[!].g";
                } else if (dict.is_ly_word(s)) {
                    e.string = s + "[!].e";
                } else {
                    e.string = s + "[!]";
                }
    ...
    private void handle_unknown_word(int i, String s) {
        ...
        word[i].x = build_word_expressions(IParserConstants.UNKNOWN_WORD);
        if (word[i].x == null) {
    ...
    public void initialize_conjunction_tables() {
        ...
        for (i = 0; i < IParserConstants.HT_SIZE; i++) {
    ...
    public void prepare_to_parse(ParseOptions opts) {
        ...
        if (!has_conjunction) {
            pp_and_power_prune(IParserConstants.RUTHLESS, opts);
        } else {
            pp_and_power_prune(IParserConstants.GENTLE, opts);
            ...
            power_prune(IParserConstants.RUTHLESS, opts);
        }
    }
    ...
    Disjunct build_COMMA_disjunct_list() {
        ...
        */
        ...
        c2.next = c3;
        c3.next = null;
        c1.priority = c3.priority = IParserConstants.DOWN_priority;
        c2.priority = IParserConstants.UP_priority;
    ...
    public void connector_for_disjunct(Disjunct d, Connector c) {
        ...
        */
        ...
        c.label = lp.label;
        c.string = d1.string;
        c.priority = IParserConstants.UP_priority;
        c.multi = false;
    }
        ...
        for (lab = 0; lab < and_data.LT_size; lab++) {
            for (d = and_data.label_table[lab]; d != null; d = d.next) {
                ...
                for (; d1 != null; d1 = d3) {
                    ...
                    c1.next = null;
                    c2.next = null;
                    c1.priority = c2.priority = IParserConstants.DOWN_priority;
    ...
    /**
     * <p>This takes a label k, modifies the list of disjuncts with that
     *      label.  For each such disjunct, it computes the string that
     *      will be used in the fat connector that represents it.</p>
     * <p>     The only hard part is finding the length of each of the strings
     *      so that "*" can be put in.  A better explanation will have to wait.</p>
     * @param label
     */
    public void compute_matchers_for_a_label(int label) {
        int lengths[];
    ...
    /**
     * <p>put the next len characters from c.string (skipping upper
     *    case ones) into s.  If there are fewer than this, pad with '*'s.
     *    Then put in a character for the multi match bit of c.
     *    Then put in a '\0', and return a pointer to this place.</p>
     * @param s 
     * @param c 
     * @param len 
     */
    ...
    /**
     * Determine if there is a conjunction between the suppled right and
     * left words. 
    ...
    private void set_connector_length_limits(ParseOptions opts) {
        ...
        if (len > IParserConstants.UNLIMITED_LEN)
            len = IParserConstants.UNLIMITED_LEN;
        for (i = 0; i < length; i++) {
    ...
    private void set_connector_list_length_limit(Connector c, ConnectorSet conset, int short_len, ParseOptions opts) {
        for (; c != null; c = c.next) {
            if (opts.parse_options_get_all_short_connectors()) {
                ...
                c.length_limit = IParserConstants.UNLIMITED_LEN;
            } else {
                c.length_limit = short_len;
            }
        }
    ...
    /**
     * <p>Step three in parsing a sentence. First you must create a dictionary, then a sentence, then call this
     * method to generate a Main tree.  This is really the heart of the system.  There are several things 
     * done when a sentence is parsed.</p>
     *<ul>
     *<li>  Word expressions are extracted from the dictionary and pruned</li>
     *<li> Disjuncts are built</li>
     *<li>  A series of pruning operations is performed.</li>
     *<li>  The linkages having the minimum number of null links are counted.</li>
     *<li>  A "Main set" of linkages is built.</li>
     *<li>  The linkages are post processed</li>
     *</ul>
     * <p>The "Main set" is attached to the sentence, and this is one of the key reasons that
     *the API is flexible and modular.  All of the necessary information for building linkages
     *is stored in the Main set.  This means that other sentences can be parsed, possibly using 
    ...
    public int sentence_parse(ParseOptions opts) {
        ...
        // A Main set may have been already been built for this sentence,
    ...
    public static int hash(int lw, int rw, Connector le, Connector re, int cost) {
        ...
        i = i + (i << 1) + MyRandom.randtable[(lw + i) & (IParserConstants.RTSIZE - 1)];
        i = i + (i << 1) + MyRandom.randtable[(rw + i) & (IParserConstants.RTSIZE - 1)];
        i =
            i
                + (i << 1)
                + MyRandom.randtable[(((le == null ? 0 : le.hashCode()) + i) % (Sentence.ctable_size + 1))
                    & (IParserConstants.RTSIZE - 1)];
        i =
            i
                + (i << 1)
                + MyRandom.randtable[(((re == null ? 0 : re.hashCode()) + i) % (Sentence.ctable_size + 1))
                    & (IParserConstants.RTSIZE - 1)];
        i = i + (i << 1) + MyRandom.randtable[(cost + i) & (IParserConstants.RTSIZE - 1)];
    ...
    public static int match_l_table_size[] = new int[IParserConstants.MAX_SENTENCE]; /* the sizes of the hash tables */
    public static int match_r_table_size[] = new int[IParserConstants.MAX_SENTENCE];

    /* the beginnings of the hash tables */
    public static MatchNode match_l_table[][] = new MatchNode[IParserConstants.MAX_SENTENCE][];
    public static MatchNode match_r_table[][] = new MatchNode[IParserConstants.MAX_SENTENCE][];
    ...
    /**
     * <p>The disjunct d (whose left or right pointer points to c) is put
           into the appropriate hash table<br />
     *
           dir =  1, we're putting this into a right table.<br />
     *
           dir = -1, we're putting this into a left table.</p
     *</p>
     * @param size 
     * @param t 
     * @param d 
    ...
    public static int fast_match_hash(Connector c) {
        String s;
        int i;
        i = MyRandom.randtable[c.label & (IParserConstants.RTSIZE - 1)];
        s = c.string;
        int j = 0;
        while (j < s.length() && Character.isUpperCase(s.charAt(j))) {
            i = i + (i << 1) + MyRandom.randtable[(s.charAt(j) + i) & (IParserConstants.RTSIZE - 1)];
            j++;
        }
        return i;
    }
    ...
    /**
     * <p>This is the top level call that computes the whole parse_set.  It
     ...
     *      must be structured just like Main() (the main function for
     *      count()).  
     * </p><p>      
     *      If the number of linkages gets huge, then the counts can overflow.
     *      We check if this has happened when verifying the Main set.
     ...
     * a new ResultLinkage object.  If you want another postprocessor this is the method
     * to call.
     * @param opts
    ...
    public void post_process_linkages(ParseOptions opts) {
        ...
        opts.print_time("Built Main set");
        if (overflowed) {
            ...
            for (in = 0; in < N_linkages_alloced; in++) {
                ...
                if (set_has_fat_down()) {
                    if (only_canonical_allowed && !is_canonical_linkage())
                        continue;
                    analyze_fat_linkage(opts, IParserConstants.PP_FIRST_PASS);
                } else {
                    analyze_thin_linkage(opts, IParserConstants.PP_FIRST_PASS);
                }
            }
        ...
        for (in = 0; in < N_linkages_alloced; in++) {
            ...
            if (set_has_fat_down()) {
                ...
                if (only_canonical_allowed && !canonical)
                    continue;
                link_info[N_linkages_post_processed] = analyze_fat_linkage(opts, IParserConstants.PP_SECOND_PASS);
                link_info[N_linkages_post_processed].fat = true;
                link_info[N_linkages_post_processed].canonical = canonical;
            } else {
                link_info[N_linkages_post_processed] = analyze_thin_linkage(opts, IParserConstants.PP_SECOND_PASS);
    ...
    public static boolean visited[] = new boolean[IParserConstants.MAX_SENTENCE];
    public static int and_element_sizes[] = new int[IParserConstants.MAX_SENTENCE];
    public static int and_element[] = new int[IParserConstants.MAX_SENTENCE];
    public static int N_and_elements;
    public static int outside_word[] = new int[IParserConstants.MAX_SENTENCE];
    ...
    public LinkageInfo analyze_fat_linkage(ParseOptions opts, int analyze_pass) {
        ...
        if (analyze_pass == IParserConstants.PP_SECOND_PASS) {
        ...
        for (;;) { /* loop through all the sub linkages */
            ...
            if (analyze_pass == IParserConstants.PP_FIRST_PASS) {
    ...
    /**
     * <p>Takes a sublinkage and returns:<br/>
     *<ul>
     * <li>      . for each link, the domain structure of that link</li>
     * <li>      . a list of the violation strings</li>
     * </ul>
     * </p>
    ...
    public void compute_pp_link_array_connectors(Sublinkage sublinkage) {
        ...
        for (end = -1; end <= 1; end += 2) {
            for (link = 0; link < pi.N_links; link++) {
                ...
                if (this_end_con.label != IParserConstants.NORMAL_LABEL)
                    continue;
                ...
                place = 0;
                if ((dis.left != null) && (dis.left.priority == IParserConstants.UP_priority)) {
                    upcon = dis.left;
                } else if ((dis.right != null) && (dis.right.priority == IParserConstants.UP_priority)) {
                    upcon = dis.right;
                } else {
                    upcon = null;
                }
                ...
                if ((mydis.left != null) && (mydis.left.priority == IParserConstants.UP_priority)) {
                    mycon = mydis.left;
                } else if ((mydis.right != null) && (mydis.right.priority == IParserConstants.UP_priority)) {
                    mycon = mydis.right;
                } else {
                    throw new RuntimeException(
    ...
    public AndList build_andlist() {
        ...
        for (w = 0; w < pi.N_words; w++) {
            ...
            if (is_conjunction[w]) {
                ...
                if (N_and_elements > 0) {
                    min = IParserConstants.MAX_SENTENCE;
                    max = 0;
                    for (i = 0; i < N_and_elements; i++) {
    ...
    public boolean is_canonical_linkage() {
        ...
        */
        ...
        dummy_connector.priority = IParserConstants.UP_priority;
        ...
        for (w = 0; w < pi.N_words; w++) {
            ...
            for (d_c = chosen_d.left; d_c != null; d_c = d_c.next) {
                if (d_c.priority == IParserConstants.DOWN_priority) {
                    d_label = d_c.label;
                    break;
                }
            }
            ...
            if ((chosen_d.left != null) && (chosen_d.left.priority == IParserConstants.UP_priority)) {
                upcon = chosen_d.left;
            } else if ((chosen_d.right != null) && (chosen_d.right.priority == IParserConstants.UP_priority)) {
    ...
    public static boolean has_fat_down[] = new boolean[IParserConstants.MAX_SENTENCE];

    /* points to the image structure for each word.  null if not a fat word. */
    public static ImageNode image_array[] = new ImageNode[IParserConstants.MAX_SENTENCE];
    ...
    */
    ...
    public void build_image_array() {
        ...
        for (end = -1; end <= 1; end += 2) {
            for (link = 0; link < pi.N_links; link++) {
                ...
                if (this_end_con.priority == IParserConstants.DOWN_priority)
                    continue;
                if ((this_end_con.label != IParserConstants.NORMAL_LABEL) && (this_end_con.label < 0))
                    continue;
                ...
                if (this_end_con.priority == IParserConstants.UP_priority) {
                    in.place = 0;
                } else {
                    in.place = 1;
                    if ((dis.left != null) && (dis.left.priority == IParserConstants.UP_priority)) {
    ...
    public boolean set_has_fat_down() { /* Fill in the has_fat_down array.  Uses link_array[].
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             ...
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             fat down label.
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          */ /* true if this word has a fat down link false otherise */
        boolean has_fat_down[] = new boolean[IParserConstants.MAX_SENTENCE];
        int link, w, N_fat;
        ParseInfo pi = parse_info;
        N_fat = 0;
        for (w = 0; w < pi.N_words; w++) {
        ...
        for (link = 0; link < pi.N_links; link++) {
            if (pi.link_array[link].lc.priority == IParserConstants.DOWN_priority) {
                N_fat++;
                has_fat_down[pi.link_array[link].l] = true;
            } else if (pi.link_array[link].rc.priority == IParserConstants.DOWN_priority) {
                N_fat++;
  ...
  private void construct_comma() {
        ...
        */
        ...
        for (w = 0; w < length - 1; w++) {
            if ((word[w].string.equals(",")) && is_conjunction[w + 1]) {
                word[w].d = Disjunct.catenate_disjuncts(special_disjunct(IParserConstants.COMMA_LABEL, '+', "", ","), word[w].d);
    ...
    private void construct_either() {
        ...
        for (w = 0; w < length; w++) {
            if (!word[w].string.equals("either"))
                continue;
            word[w].d =
                Disjunct.catenate_disjuncts(special_disjunct(IParserConstants.EITHER_LABEL, '+', "", "either"), word[w].d);
        }

        for (w = 0; w < length; w++) {
            if (!word[w].string.equals("or"))
                continue;
            word[w].d = glom_aux_connector(word[w].d, IParserConstants.EITHER_LABEL, false);
        }
    }
    ...
    private void construct_neither() {
        ...
        for (w = 0; w < length; w++) {
            if (!word[w].string.equals("neither"))
                continue;
            word[w].d =
                Disjunct.catenate_disjuncts(special_disjunct(IParserConstants.NEITHER_LABEL, '+', "", "neither"), word[w].d);
        }

        for (w = 0; w < length; w++) {
            if (!word[w].string.equals("nor"))
                continue;
            word[w].d = glom_aux_connector(word[w].d, IParserConstants.NEITHER_LABEL, true);
        }
    }
    ...
    private void construct_notonlybut() {
        ...
        for (w = 0; w < length; w++) {
            if (!word[w].string.equals("not"))
                continue;
            word[w].d = Disjunct.catenate_disjuncts(special_disjunct(IParserConstants.NOT_LABEL, '+', "", "not"), word[w].d);
            if (w < length - 1 && word[w + 1].string.equals("only")) {
                word[w + 1].d =
                    Disjunct.catenate_disjuncts(special_disjunct(IParserConstants.NOTONLY_LABEL, '-', "", "only"), word[w + 1].d);
                d = special_disjunct(IParserConstants.NOTONLY_LABEL, '+', "", "not");
                d = add_one_connector(IParserConstants.NOT_LABEL, '+', "", d);
        ...
        for (w = 0; w < length; w++) {
            if (!word[w].string.equals("but"))
                continue;
            word[w].d = glom_aux_connector(word[w].d, IParserConstants.NOT_LABEL, false);
    ...
    private void construct_both() {
        ...
        for (w = 0; w < length; w++) {
            if (!word[w].string.equals("both"))
                continue;
            word[w].d = Disjunct.catenate_disjuncts(special_disjunct(IParserConstants.BOTH_LABEL, '+', "", "both"), word[w].d);
        }

        for (w = 0; w < length; w++) {
            if (!word[w].string.equals("and"))
                continue;
            word[w].d = glom_aux_connector(word[w].d, IParserConstants.BOTH_LABEL, false);
        }
    }
    ...
    */
    ...
    public static Disjunct glom_comma_connector(Disjunct d) {
        ...
        */
        ...
        for (d1 = d; d1 != null; d1 = d1.next) {
            ...
            c1.init_connector();
            c1.string = "";
            c1.label = IParserConstants.COMMA_LABEL;
            c1.priority = IParserConstants.THIN_priority;
            c1.multi = false;
            c1.next = null;
    ...
    public static Disjunct add_one_connector(int label, int dir, String cs, Disjunct d) {
        ...
        c.string = cs;
        c.label = label;
        c.priority = IParserConstants.THIN_priority;
        c.multi = false;
        c.next = null;
 ...
 <p>    
  ...
  <p>   
    ...
    */
    ...
    public static int power_l_table_size[] = new int[IParserConstants.MAX_SENTENCE];
    public static int power_r_table_size[] = new int[IParserConstants.MAX_SENTENCE];

    /* the beginnings of the hash tables */
    public static CList power_l_table[][] = new CList[IParserConstants.MAX_SENTENCE][];
    public static CList power_r_table[][] = new CList[IParserConstants.MAX_SENTENCE][];
 ...
 *      deletable, this is equivalent to RUTHLESS.   --DS, 7/97
    ...
    public int power_prune(int mode, ParseOptions opts) {
        ...
        while (true) {
            ...
            for (w = 0; w < length; w++) {
                for (d = word[w].d; d != null; d = d.next) {
                    ...
                    if (left_connector_list_update(d.left, w, w, true) < 0) {
                        for (c = d.left; c != null; c = c.next) {
                            c.word = IParserConstants.BAD_WORD;
                        }
                        for (c = d.right; c != null; c = c.next) {
                            c.word = IParserConstants.BAD_WORD;
                        }
                ...
                for (d = word[w].d; d != null; d = dx) {
                    dx = d.next;
                    if ((d.left != null) && (d.left.word == IParserConstants.BAD_WORD)) {
                        d.next = free_later;
                        free_later = d;
                    } else {
                        d.next = nd;
            ...
            for (w = length - 1; w >= 0; w--) {
                ...
                for (d = word[w].d; d != null; d = dx) {
                    dx = d.next;
                    if ((d.right != null) && (d.right.word == IParserConstants.BAD_WORD)) {
                        d.next = free_later;
                        free_later = d;
                    } else {
                        d.next = nd;
        ...
        if (mode == IParserConstants.RUTHLESS) {
    ...
    public void clean_table(int size, CList t[]) {
        ...
        for (i = 0; i < size; i++) {
            ...
            for (m = t[i]; m != null; m = xm) {
                xm = m.next;
                if (m.c.word != IParserConstants.BAD_WORD) {
                    m.next = head;
                    head = m;
                }
            }
    ...
    * take this connector list, and try to match it with the words
    ...
    public int left_connector_list_update(Connector c, int word_c, int w, boolean shallow) {
        ...
        foundmatch = false;
        for (;(n >= 0) && ((w - n) <= IParserConstants.MAX_SENTENCE); n--) {
            power_cost++;
    ...
    public int right_connector_list_update(Connector c, int word_c, int w, boolean shallow) {
        ...
        foundmatch = false;
        for (;(n < length) && ((n - w) <= IParserConstants.MAX_SENTENCE); n++) {
            power_cost++;
    ...
    public boolean possible_connection(Connector lc, Connector rc, boolean lshallow, boolean rshallow, int lword, int rword) {
        ...
        if (power_prune_mode == IParserConstants.RUTHLESS) {
            if (lword == rword - 1) {
